...
func main() {
	...
	tenantUcase := usecase.NewTenantUsecase(tenantRepo, userRepo, redisClient, cfg)
	productUsecase := usecase.NewProductUsecase(productRepo)
	saleUsecase := usecase.NewSaleUsecase(saleRepo, productRepo, financeRepo)
	...
	notificationUsecase := usecase.NewNotificationUsecase(productRepo, saleRepo)

	// 6. Initialize Handlers
	tenantHnd := handler.NewTenantHandler(tenantUcase)
	productHnd := handler.NewProductHandler(productUsecase)
	saleHnd := handler.NewSaleHandler(saleUsecase)
	customerHnd := handler.NewCustomerHandler(customerUsecase)
	inventoryHnd := handler.NewInventoryHandler(inventoryUsecase)
	financeHnd := handler.NewFinanceHandler(financeUsecase)
	notificationHnd := handler.NewNotificationHandler(notificationUsecase)
