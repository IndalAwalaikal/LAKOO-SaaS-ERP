...
func NewFinanceHandler(uu usecase.FinanceUsecase) *FinanceHandler {
	return &FinanceHandler{usecase: uu}
}
...
func (h *FinanceHandler) Record(c *gin.Context) {
	...
	res, err := h.usecase.RecordTransaction(tenantID.(string), userID.(string), &req)
	if err != nil {
...
func (h *FinanceHandler) Delete(c *gin.Context) {
	...
	err := h.usecase.DeleteTransaction(id, tenantID.(string))
	if err != nil {
...
func (h *FinanceHandler) List(c *gin.Context) {
	...
	txs, err := h.usecase.GetTransactions(tenantID.(string))
	if err != nil {
...
func (h *FinanceHandler) Summary(c *gin.Context) {
	...
	summary, err := h.usecase.GetSummary(tenantID.(string), startDate, endDate)
	if err != nil {
		response.Error(c, 500, "INTERNAL_SERVER_ERROR", err.Error())
		return
	}
