...
func NewSaleUsecase(sr repository.SaleRepository, pr repository.ProductRepository, fr repository.FinanceRepository) SaleUsecase {
	return &saleUsecase{
		saleRepo:    sr,
		productRepo: pr,
		financeRepo: fr,
...
func (u *saleUsecase) CreateSale(tenantID string, cashierID string, req *dto.SaleRequest) (*domain.Sale, error) {
	...
	financeRecord := &domain.FinanceTransaction{
		ID:          uuid.New().String(),
		TenantID:    tenantID,
		Type:        "Income",
	...
	err = u.financeRepo.Create(financeRecord)
	if err != nil {
...
func (u *saleUsecase) ListSales(tenantID string) ([]domain.Sale, error) {
	return u.saleRepo.FindByTenant(tenantID)
}

func (u *saleUsecase) GetSalesTrend(tenantID string) ([]dto.SalesTrendResponse, error) {
